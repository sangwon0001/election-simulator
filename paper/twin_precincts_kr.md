...
재현되지 않았고, 근접쌍 밀도비 r(d)와 투표유형 구성까지 모형 예측과 일치했다. 넷째, 유형
...
and vote-type composition matching the model. Type decomposition localizes the local-election
...
4. **반복 검증 설계**: "초과가 구조적이면 다른 선거에서도 나타난다"는 사전지정 예측을
   2025 대통령선거로 검정하여 기각한다. 이는 단일 선거 사후분석의 순환성을 부분적으로
   해소한다.
...
재독으로 검증할 수 없다 — 이는 물리적 재검표의 영역이다. (5) 직전 지방선거(2022)
반복 분석은 미완이다 — "지방선거 일반 대 2026 한정"의 판별에 해당하며, 본 결론의
방향을 바꾸지는 않으나 잔여 이상의 해석을 좁힐 것이다.

### 7.3 향후 작업

(a) 제8회 지방선거(2022) 동일 분석, (b) 국회의원선거(소선거구 단위) 확장, (c) 사이드밴드
추정의 일반화 — 임의 차원 다변량 정확일치, 이질적 노이즈 척도 하의 셸 가중.
...
대통령선거에서는 초과가 재현되지 않았다. 정확일치는 매 선거 발생하는 자연 현상이고,
...
`bias_demo.py`(명제 1 수치검증), `type_decompose.py`(유형 분해), `abinitio_sim.py`
(비조건부 변형), `verify_twins.py`(자료 무결성, §2.4), `paper/make_figs.py`(그림).
크롤 원자료와 정제 데이터 동봉.
